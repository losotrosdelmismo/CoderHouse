...
io.on('connect', socket => {
    ...
    socket.on('producto-nuevo', p => {        
        p.id = data.length + 1;
        data.push(p);   
        fs.writeFileSync('./baseDeDatos/productos.json', JSON.stringify(data, null, '\t'));             
        io.sockets.emit('productos', data);
    });
    //Mensajes del chat
    socket.on('nuevo-mensaje', mensaje => {
        mensajes.push(mensaje);
        fs.writeFileSync('./baseDeDatos/mensajes.txt', JSON.stringify(mensajes, null, '\t'));    
    ...
    socket.on('articuloCarrito', p => {                    
        ...
        fs.writeFileSync('./baseDeDatos/carrito.txt', JSON.stringify(carrito, null, '\t'));
    })
});
...
productosRouter.get('/listar', async (req, res) =>{
    try{
        ...
        console.log('fallo el listar productos')
    }
})
...
//ruta que agregue para que funcione el formulario de agregar
