...
//GraphQl
if(dotenv.graphql){
    const graphiql = require('./utils/graphql');
    var { graphqlHTTP } = require('express-graphql');


    app.use('/graphql', graphqlHTTP({
        schema: graphiql.schema,
        rootValue: graphiql.root,
        graphiql: true
    }));
}
...
async function cargaDatos(){
    try{       
        ...
        productos = await mongoCrud.leer(productos, "productos");
        chat = await mongoCrud.leer(chat, "chat");                      
    }
    catch(error){
        logger.error(error);
    }
...
//----WEB SOCKET-----------
io.on('connect', socket => {                           
    socket.on('compra-finalizada', pedido => {
        //enviamos whatsapp
        ...
        nodemailer.createMailOptions('compra', pedido);
    })
    socket.on('nuevo-mensaje', mensaje => {
        ...
        (async () => {
            try{    
               await mongoCrud.guardar(chat, "chat");                
            }catch(error){
...
http.listen(PORT, () => {
    logger.warn(`servidor escuchando en http://localhost:${PORT} graphQl: ${dotenv.graphql}`);
});
http.on('error', error => {
