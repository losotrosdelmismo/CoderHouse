...
app.get('/login', async (req, res) => {  
 ...
 io.on('connect', socket => {
  ...
  socket.on('envio-mail', mailOptions => {
      
    createMailOptions('login', 'losotrosdelmismo@gmail.com');
    createMailOptions('login', 'reese.jast@ethereal.email');
  })
...
app.get('/logout', (req, res) => {   
    createMailOptions('logout', 'losotrosdelmismo@gmail.com');
    createMailOptions('logout', 'reese.jast@ethereal.email');
...
function createMailOptions(operacion, mail){
    let mailOptions;
    let titulo;
    let mensaje;
    ...
    if(operacion == 'login'){
        titulo = 'Usuario login'
        mensaje = `<h1 style="color: green;">El usuario ${usuario.displayName} fue logueado exitosamente</h1><h2>fecha de ingreso: ${new Date().toLocaleString()}</h2> 
        <div> <img src=${picture} alt="">`;
    }

    if(operacion == 'logout'){
       titulo = 'usuario logout'; 
       mensaje = `<h1 style="color: red;">El usuario ${usuario.displayName} hizo log out</h1><h2>fecha de salida: ${new Date().toLocaleString()}</h2> 
        <div> <img src=${picture} alt="">`;        
...
//----TWILIO SMS-----------
const twilioSMS = require('./utils/sendSMS');

function sendSMS(mensaje){
    twilioSMS.messages.create({
        body: 'el usuario: '+ mensaje.autor + ' escribio: ' + mensaje.texto,
        from: '+14098775700',
        to: '+541140832701'
  })
