...
app.get('/', async (req, res) => {
    data = await productos.leer(data);
        if(data.length == 0){
            res.render('layouts', { productos: data , hayProductos: false });
        }
        else{
           
            res.render('layouts', { productos: data , hayProductos: true });
...
router.delete('/borrar/:id', async (req, res) =>{
    ...
    if(req.params.id > data.length || req.params.id == 0){
      ...
      productos.actualizarProductos(data)
      res.render('layouts', { productos: borrado , hayProductos: true });
    }    
})